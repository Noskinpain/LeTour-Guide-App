...
const Header = ({setCoordinates}) => {
    ...
    return ( 
        <div className="flex items-center w-full p-2 absolute top-0 z-10">
            <div className="flex bg-black justify-between items-center w-full p-3 md:p-4 rounded-sm shadow-md">
                {/* Logo displays only when  */}
                { !search && ( 
                    <img src="" alt="LeTour Guide" className="h-6 sm:h-7 md:h-8 font-bold text-white uppercase text-2xl" />
                ) }
                {/* --- */}
                ...
                <div className={`relative w-full md:w-auto md:block ${!search && 'hidden'}`}>
                    ...
                    <Autocomplete onLoad={onLoad} onPlaceChanged={onPlaceChanged}>
                        <div className="group relative">
                            <svg className="absolute left-3 top-1/2 -mt-2.5 text-black pointer-events-none group-focus-within:text-black" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                            </svg>
                            <input 
                                className="focus:ring-2 focus:ring-black focus:outline-none appearance-none w-[400px] text-sm leading-6 text-slate-900 placeholder-slate-700 rounded-sm py-1 md:py-2 pl-10 ring-1 ring-slate-200" 
                                type="text" 
